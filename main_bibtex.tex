...
%  BibTeX variant. Compile: pdflatex; bibtex; pdflatex; pdflatex
% ============================================================
...
\date{July 7, 2026\\[4pt]
\small Code and data: \url{https://github.com/rifmj/helical-fields}\quad
DOI: \href{https://doi.org/10.5281/zenodo.21246333}{10.5281/zenodo.21246333}}
...
\begin{figure}[h]
  ...
  \caption{Two independent authoring axes on a single divergence-free field.
    ...
    power spectrum}. \textbf{Top to bottom:} the helicity knob $p$ (\S\ref{sec:helicity})
    sweeps the chirality from right- to left-handed. The two are orthogonal: across
    every row the total energy is identical to four significant figures, and the net
    helicity $\int\bu\cdot\bomega$ is set by $p$ alone ($+3.1\cdot10^{-4}$, $0$,
    $-3.1\cdot10^{-4}$ for the three rows) and is unchanged by $\lambda$ to machine
...
procedural animation of smoke, particles, and other advected media, with three mutually
independent controls: spectral slope, helicity, and phase coherence. The construction works
in the frequency domain on the \emph{helical} (curl-eigenvector) basis, which is
divergence-free by construction and reduces helicity to a single polarization parameter,
continuous from mirror-symmetric to maximally helical (Beltrami). The phase-coherence
control is, to our knowledge, new to procedural flow: it interpolates the inter-mode phases
between random and structured \emph{at an exactly fixed power spectrum}, carrying the field
from unstructured noise to organized filaments without changing its energy content---an
axis that curl-noise does not expose. We prove that the real-field (Hermitian)
symmetrization preserves the helicity polarization, give the construction in two and three
dimensions, and provide two evaluation paths: an FFT bake and an analytic Beltrami mode-sum
that evaluates per pixel in a fragment shader (GLSL included). Every number in the paper is
reproduced by a pinned-seed script (\texttt{make reproduce}).
\end{abstract}
...
knobs---frequency and amplitude. Two qualities that distinguish real turbulent flow are not
directly controllable:
\begin{itemize}[nosep]
\item \textbf{Helicity} (chirality): whether the vortices of the flow prefer a handedness.
Helicity, $\int \bu\cdot\bomega\,d\bx$, is a genuine three-dimensional invariant of ideal
flow~\cite{moffatt1969}; a field can range from mirror-symmetric to fully helical
(a Beltrami field, $\bomega\parallel\bu$)~\cite{constantin1988,dombre1986}.
\item \textbf{Phase organization}: at a \emph{fixed} power spectrum, a field with random
inter-mode phases is featureless (Gaussian), whereas correlated phases concentrate energy
...
(\S\ref{sec:modesum});
\item numerical validation of divergence-freeness, reality, and helicity control for both
constructions (\S\ref{sec:validation}).
\end{enumerate}
...
\begin{figure}[t]
  ...
  \caption{The chirality made concrete on a single Beltrami mode
    ...
    $\int\bu\cdot\bomega$ tracks $p$ in sign and magnitude ($\mp2.6\cdot10^{-3}$ at
    $p=\mp1$, $\approx0$ at $p=0$ on a $32^3$ grid).}
  \label{fig:helix}
\end{figure}
...
Graphics needs a \emph{real} field. Building modes with complex amplitudes yields a complex
...
distinct rolling vortices out of the haze.)
...
We report three experiments, each reproduced by a self-contained script accompanying the
paper (\texttt{code/e1..e3}). The printed numbers below are exactly those the scripts emit at
the pinned seed.
...
indistinguishable from it (equal-spectrum speed flatness $2.00\pm0.02$ for both, versus
$5.1$ at $\lambda=1$; Fig.~\ref{fig:curlnoise}). The FFT path shares curl-noise's per-frame
...
\paragraph{Conclusion.} Building divergence-free fields on the helical basis turns two
otherwise-inaccessible qualities of a flow---its chirality and its degree of organization at
fixed spectrum---into two independent, continuously variable controls, while keeping
divergence-freeness exact. The constructions are elementary and reproducible; the accompanying
code regenerates every number reported above.
