...
\begin{figure}[htbp]
  ...
  \caption{Two independent authoring axes on a single divergence-free field.
    ...
    right-handed (top) through balanced (middle) to left-handed (bottom). Chirality is a
    correlation, not a colour: in a helical field the in-plane swirl and the out-of-plane
    velocity (the colour, $u_z$) are locked in a definite sense, and that sense reverses
    between the top and bottom rows, while the middle row has no preferred twist. The two
    axes are orthogonal: the total energy is identical across every row (it does not
    depend on $p$ at all), and the net helicity $\int\bu\cdot\bomega$ is set by $p$
    alone---exactly antisymmetric, $+1.3\cdot10^{-3}$, $0$, $-1.3\cdot10^{-3}$ for the
    three rows---and is unchanged by $\lambda$ to machine precision. Colour is the
    out-of-plane velocity $u_z$; black curves are in-plane streamlines; each panel is a
    $z$-slice of a $48^3$ field.}
  \label{fig:teaser}
\end{figure}
...
preferred twist) to maximally helical (every mode fully corkscrewed---Beltrami mode by
mode). The
...
and how tightly it organizes---qualities that ordinary curl-noise leaves fixed---at no added
solver cost downstream.
\end{abstract}
...
no-penetration---for any obstacle with a smooth signed distance, with no solver
(\S\ref{sec:obstacles});
...
graphics~\cite{stam1993,kim2008}. Synthesizing divergence-free random fields with a
prescribed spectrum is itself long established outside graphics---kinematic simulation
sums unsteady random Fourier modes constrained to be solenoidal~\cite{fung1992}, and
divergence-free synthetic-turbulence generators build the field as the curl of a random
vector potential with a target spectrum~\cite{saad2017}. Our contribution is not that
machinery but the pair of \emph{artist-facing} controls it does not expose---a single
helicity/chirality dial and a phase-coherence dial at fixed spectrum---together with the
in-shader mode-sum and the potential-ramp obstacle treatment. \S\ref{sec:discussion}
positions the method against curl-noise and states its limits.
...
\label{eq:env}
\end{equation}
with slope $s$ and a smooth high-frequency cutoff $k_c$. Here $E$ is the \emph{amplitude}
envelope, $|\hat\bu(\bk)|\propto E(\bk)$, so the modal energy scales as $|\bk|^{-2s}$ and the
energy in a $d$-dimensional shell as $|\bk|^{d-1-2s}$; we quote $s$ as the amplitude exponent
throughout. (In three dimensions the envelope is placed directly on the velocity; in the 2D
stream-function path~\eqref{eq:2d} it is placed on the vorticity, so the velocity there
carries an extra $|\bk|^{-1}$ and a given $s$ means a slope steeper by one. We keep the two
parametrizations because each is the natural one in its construction.) Steep $s$ concentrates
energy in a few large scales (smooth, silky flow); shallow $s$ spreads it across scales (fine,
ragged flow) (Fig.~\ref{fig:slope}).
...
\label{eq:2d}
\end{equation}
with the sign convention $\omega=\partial_x v-\partial_y u=-\Delta\psi$, which satisfies
$\bk\cdot\hat\bu(\bk)=0$ identically. There is no helicity in two dimensions;
...
energy is in one channel and the field is purely helical: $\bomega=\pm|\bk|\,\bu$
mode by mode, so $\bomega\parallel\bu$ within each shell (a genuine Beltrami field only
for a single shell; across shells the alignment is imperfect, \S\ref{sec:validation}). The
ABC flows~\cite{dombre1986} are the classical single-shell example
(Fig.~\ref{fig:helix}).
...
\begin{equation}
a_\pm(\bk)\ \longmapsto\ e^{\,i\varphi_\pm(\bk)}\,a_\pm(\bk),
\qquad
\varphi_\pm(\bk)=\theta_{\mathrm{ref}}^{\pm}(\bk)+(1-\lambda)\,\theta_\pm(\bk),
\quad \lambda\in[0,1].
\label{eq:phase}
\end{equation}
Because the moduli $|a_\pm(\bk)|$ are untouched, the entire \emph{helical energy spectrum}---the
power spectrum $|a_+|^2+|a_-|^2$ \emph{and} the polarization split~\eqref{eq:split}, hence the
helicity---is frozen \emph{exactly} at every $\lambda$; only the phases move. Here
$\theta_{\mathrm{ref}}^{\pm}$ are structured reference phases (one per channel) and
$\theta_\pm$ are independent uniform random phases, all drawn antisymmetric
($\theta(-\bk)=-\theta(\bk)$) so the field stays real and (transversality being untouched)
divergence-free. At $\lambda=1$ each channel locks onto its reference; at $\lambda=0$ they carry
independent i.i.d.\ phases (an isotropic Gaussian haze, the RPA surrogate of the same
amplitudes). Using \emph{distinct} references for the two channels keeps the balanced field
($p=0$) isotropic at every $\lambda$: a single shared reference would collapse
$a_+$ and $a_-$ to the same phase at $\lambda=1$ and linearly polarize the $p=0$ field. In two
dimensions there is a single scalar amplitude, so \eqref{eq:phase} reduces to a common per-mode
phase $e^{\,i(\theta_{\mathrm{ref}}+(1-\lambda)\theta)}$ on the stream function (used in E4). Sliding
$\lambda$ carries the field from formless noise to organized filaments \emph{without changing
the energy at any scale}---the axis that curl-noise lacks. (In the demonstrations of \S\ref{sec:validation} the reference
...
drawn with $\Pr(s_j=+1)=(1+p)/2$. Because the directions are sampled uniformly on the
sphere rather than on the integer lattice, the mode-sum does not reproduce the FFT field's
exact per-shell energy; matching a target shell spectrum requires weighting the draws by the
lattice mode density ($\propto|\bk|^{d-1}$) or sampling $|\bk_j|$ from it. This is one facet
of the sparse-versus-dense trade-off (\S\ref{sec:discussion}). Each term is the real part of a single helical mode: it is
...
Two properties hold \emph{by construction}, wherever the signed distance is
smooth (so that $\nabla d$ exists---away from the medial axis and any edges or
corners of the obstacle, a measure-zero set the ramp never needs to resolve).
First, $\bu_b$ is a curl, so it is exactly divergence-free: classically outside
the surface, and distributionally across it (the field is zero inside $d<0$ and a
tangential slip flow just outside, so there is a velocity discontinuity at $d=0$,
as for any free-slip wall---see below). Second, it is tangent to the surface: on
$d=0$ the first term of~\eqref{eq:bounded} carries the factor $\nabla d\times\bA$,
whose component along the surface normal $\hat\bn=\nabla d$ is
$(\nabla d\times\bA)\cdot\nabla d=0$ identically, while the second term carries the
factor $\alpha(0)=0$; hence $\bu_b\cdot\hat\bn=0$ on the wall. Because the wall
normal velocity vanishes identically, $\{d=0\}$ is an invariant surface of the
exact flow: a continuous trajectory cannot cross it, so containment is a theorem
for the continuous field and only the time integrator can leak (E6 measures that
leak). This is precisely the free-slip, boundary-respecting behaviour targeted by
pointwise incompressible interpolation for grid-based fluids~\cite{chang2022},
obtained here in closed form on the analytic field with no grid and no projection.
The ramp $\alpha$ is $C^2$ at the band edge $q=1$ but only one-sided at the wall
($\alpha'(0^-)=0\neq\tfrac{15}{8}=\alpha'(0^+)$), which is exactly the slip
discontinuity noted above. Beyond the band ($d\ge\theta$) the field is
...
We report six experiments (E1--E5 and E7), each reproduced by a self-contained script
accompanying the paper (\texttt{code/e1\_*..e5\_*}, \texttt{code/e7\_*}); the obstacle
experiment E6 is reported in \S\ref{sec:obstacles} (\texttt{code/e6\_obstacle.py}). E4
validates the phase-coherence axis on the 2D construction and E7 on the 3D helical builder
that also generates the figures. The printed numbers below are exactly those the scripts emit
at the pinned seed.
...
$s=1.4$). As the coherence knob $\lambda$ is swept $0\to1$ under the phase
interpolation~\eqref{eq:phase}, the total spectral energy is invariant to
$\sum|\hat\bu|^2 = 5.209353$ at every $\lambda$ (spread $8.9\cdot10^{-16}$,
relative $1.7\cdot10^{-16}$), and the per-shell energy spectrum is likewise
frozen (maximum relative deviation of any shell across $\lambda$ is
$1.8\cdot10^{-16}$); the field stays divergence-free
($\max|\bk\cdot\hat\bu|/|\hat\bu|\le1.4\cdot10^{-17}$) and real
($\max|\mathrm{Im}\,\bu|\sim10^{-7}$---the phase-extraction floor of the reference
blobs, where $\arg$ of a near-null mode is ill-conditioned, not machine
$\epsilon$; the advected field is its real part regardless). The spatial organization,
by contrast, changes monotonically: the flatness (kurtosis) of the vorticity,
which is $\approx3$ for a random-phase field and grows as energy concentrates
into sparse coherent structures, rises
\[
  \mathrm{flatness}(\omega)\;=\;2.75,\;2.88,\;3.05,\;5.35,\;9.22
  \qquad\text{for }\lambda=0,\,0.25,\,0.5,\,0.75,\,1
\]
(16-draw mean; the $\lambda=0$ value sits just below $3$ because the
fixed-amplitude modes make the random-phase surrogate mildly sub-Gaussian), from
a near-Gaussian haze at $\lambda=0$ up to strongly intermittent at $\lambda=1$.
Structure appears; energy per scale does not move.

\paragraph{(E7) The same, in 3D, on the figure-generating builder
  \textnormal{(\texttt{e7\_phase\_coherence\_3d.py})}.}
E4 exercises the 2D stream-function path; E7 runs the identical test on the 3D
helical builder that generates the teaser and advection figures
(\texttt{\_helical.build\_fft\_3d}), so the figure code is itself under test.
On a $48^3$ grid ($s=1.4$, $p=0.6$), sweeping $\lambda=0\to1$ under~\eqref{eq:phase}
holds the total energy fixed to $\sum|\hat\bu|^2=36.059$ \emph{exactly} (relative
spread $2.0\cdot10^{-16}$), holds the net helicity fixed to
$\mathcal H=7.85\cdot10^{-4}$ (relative spread $1.4\cdot10^{-16}$) and the per-shell
spectrum fixed (max deviation $1.7\cdot10^{-16}$), and keeps the field
divergence-free ($\le9\cdot10^{-17}$) and real ($\sim4\cdot10^{-20}$). The spatial
organization again grows monotonically---the vorticity-magnitude flatness (the
$|\bomega|$ analogue of the E4 statistic; $\approx3.1$ is the Gaussian baseline for
the magnitude of three components) rises
\[
  \mathrm{flatness}(|\bomega|)\;=\;3.11,\;3.33,\;10.88,\;23.35,\;26.70
  \qquad\text{for }\lambda=0,\,0.25,\,0.5,\,0.75,\,1
\]
(8-draw mean)---while the field's relative helicity at $p=+1$ is
$\rho\approx0.77$, matching the FFT construction of E2, and the balanced field
($p=0$) stays isotropic at every $\lambda$. This is the check whose
absence let an earlier 3D builder ship with the coherence knob inoperative
(it applied a common phase on top of already-random amplitudes, so
$\mathrm{flatness}(|\bomega|)$ stayed flat at $\approx3.1$ for every $\lambda$); the
present builder (independent per-channel phases, each locking onto its own
structured reference, \eqref{eq:phase}) makes coherence respond to $\lambda$ while
freezing spectrum and helicity.
...
at $64^3$) and advection downstream carries no solver cost. The mode-sum path trades the grid for
...
\S\ref{sec:coherence}) as continuous knobs. Our $\lambda=0$ field sits at the
random-phase, mirror-symmetric, isotropic corner of this space, which is the same corner
curl-noise occupies---both are featureless at a fixed spectrum (equal-spectrum vorticity
flatness $\approx3.0$, the Gaussian value, versus $\approx11.9$ at $\lambda=1$;
Fig.~\ref{fig:curlnoise}). We stress that
this is a comparison against a \emph{random-phase spectral surrogate} built at the matched
spectrum, not against a specific curl-of-Perlin implementation, whose lattice and octave
structure imposes its own weak phase correlations; the point is only that the featureless
end of the coherence axis is curl-noise-\emph{like}, and that $\lambda$ opens an axis
curl-noise leaves fixed. The FFT path shares curl-noise's per-frame
...
\begin{table}[t]
  ...
  \caption{Curl-noise and the present construction both produce exactly
    ...
    isotropic setting of our method occupies the same featureless corner as curl-noise
    (Fig.~\ref{fig:curlnoise}).}
  \label{tab:curlnoise}
\end{table}
...
\begin{figure}[htbp]
  ...
  \caption{The random-phase corner ($\lambda=0$, left; the curl-noise-like, spectrally
    matched surrogate---see text) and our construction at $\lambda=1$ (right), built on the
    \emph{same} power spectrum and the same random seed, so that only the inter-mode phases
    differ. Colour is speed $|\bu|$, white curves are streamlines. The random phases
    give a homogeneous field (vorticity flatness $\approx3.0$, the Gaussian value); the
    structured phases concentrate the same energy into distinct coherent vortices (vorticity
    flatness $\approx11.9$) without changing the spectrum---the organization axis curl-noise
    does not expose. (Speed flatness, by contrast, barely moves between the two panels; the
    reorganization shows in the vorticity, as in E4.)}
  \label{fig:curlnoise}
\end{figure}
...
projection---the expensive step of a fluid solver---so the two new controls add no solver
cost to the downstream renderer (the advection itself, of course, still costs what advection
costs).
...
\bibitem{fung1992}
J.~C.~H.~Fung, J.~C.~R.~Hunt, N.~A.~Malik, and R.~J.~Perkins.
\newblock Kinematic simulation of homogeneous turbulence by unsteady random Fourier modes.
\newblock {\em Journal of Fluid Mechanics} 236:281--318, 1992.
\newblock \href{https://doi.org/10.1017/S0022112092001423}{doi:10.1017/S0022112092001423}.

\bibitem{saad2017}
T.~Saad, D.~Cline, R.~Stoll, and J.~C.~Sutherland.
\newblock Scalable tools for generating synthetic isotropic turbulence with arbitrary spectra.
\newblock {\em AIAA Journal} 55(1):327--331, 2017.
\newblock \href{https://doi.org/10.2514/1.J055230}{doi:10.2514/1.J055230}.
